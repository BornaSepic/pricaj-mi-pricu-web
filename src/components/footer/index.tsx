...
export const Footer = () => {
    ...
    return (
        <>
            <div className={`${styles.slideUpMenu} ${menuOpen ? styles.menuOpen : ''}`}>
                <nav className={styles.menuContent}>
                    <ul>
                        {departments.map((department) => (
                            <li key={department.id}>
                                <Link href={`/department/${department.id}`} onClick={() => setMenuOpen(false)}>
                                    {department.name}
                                </Link>
                            </li>
                        ))}
            ...
            <footer className={clsx(styles.footer, menuOpen && styles.active)}>
                <div className={`${styles.logoContainer} ${menuOpen ? styles.active : ''}`} onClick={() => setMenuOpen(!menuOpen)}>
